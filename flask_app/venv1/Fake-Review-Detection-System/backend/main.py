...
def batch_predict(body: BatchRequest):
    """Process up to 30 reviews in a single batch request."""
    t0=time.perf_counter()
    ...
    if len(reviews) > 30:
        raise HTTPException(status_code=400, detail="Batch size exceeds 30 reviews")
    ...
    for review_obj in reviews:
        ...
        is_valid, msg = validate_review(review_obj.review)
        
        if not is_valid:
            results.append({
                "review": review_obj.review[:200],
                "is_valid": False,
                "error": msg,
                "prediction": "Invalid",
                "verdict": "invalid"
            ...
            valid_count += 1
            # Classify valid review
            meta = review_obj.model_dump()
            meta.pop("review", None)
            if review_obj.max_similarity is not None:
                meta["Maximum Content Similarity"] = review_obj.max_similarity
            
            result = do_classify(review_obj.review, meta)
            result["review"] = review_obj.review[:200]
            result["is_valid"] = True
            result["verdict"] = "fake" if result["prediction"] == "Fake" else "genuine"
    ...
    return {
        "total": len(reviews),
        "valid": valid_count,
        "invalid": len(reviews) - valid_count,
        "fake": fake_count,
